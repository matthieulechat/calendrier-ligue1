...
- [ ] Script planifié via PM2 (`cron_restart`) sur le Raspberry Pi 5, en réutilisant l'infra PM2 déjà en place pour d'autres tâches planifiées — récupère **scores ET dates mises à jour**, fréquence à définir par compétition (ex. lundi pour Ligue 1) — `ecosystem.config.cjs` écrit (cron lundi 6h, `--push`), reste à déployer sur le Pi (clé de déploiement SSH, `.env.local`, `pm2 start`)
