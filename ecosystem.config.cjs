...
// 2. `.env.local` avec FOOTBALL_DATA_ORG_KEY (+ UPTIME_PUSH_URL optionnel)
  ...
  apps: [
    {
      ...
      // .bin/tsx est un shim shell (#!/bin/sh) : sans ça, PM2 tente de le
      // charger avec `node` et plante sur une SyntaxError.
      interpreter: "none",
      cwd: __dirname,
      ...
      // stop_exit_codes:[0] est indispensable : sans ça, PM2 relance le
      // script en boucle même quand il termine avec succès (script one-shot,
      // pas un daemon) — vécu en prod le 2026-07-18, boucle stoppée à la main.
      cron_restart: "0 6 * * 1",
      autorestart: true,
      stop_exit_codes: [0],
      max_restarts: 3,
